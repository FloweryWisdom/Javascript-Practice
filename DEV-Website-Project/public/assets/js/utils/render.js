...
// --- "Private" Helper Function for this Module ---
// This function is defined first and does not need to be exported.
function createAuthorDropdownHTML(author) {
    // If for some reason there's no author, return an empty string.
    if (!author) return '';

    // Prepare data with fallbacks
    const profilePic = author.profilePictureUrl || '/assets/images/home/main/post-profile-picture-1.webp';
    ...
    // Return the complete HTML structure for the dropdown
    return `
        <div class="dropdown">
            <p class="user-info-username"> <strong> <a href="/profile-page.html?id=${author._id}" class="dropbtn text-decoration-none text-reset">${username}</a></strong></p>
            <div class="dropdown-content rounded overflow-hidden">
                ...
                <div class="dropdown-user-data d-flex">
                    <div class="user-data-profile-pic"> <a href="/profile-page.html?id=${author._id}"> <img src="${profilePic}" alt="${username}"></a></div>
                    <p class="user-data-profile-name-container m-0" ><a class="user-data--profile-name" href="/profile-page.html?id=${author._id}"><b>${username}</b></a></p>
                </div>
                <button class="user-data-button btn btn-primary">Follow</button>
                <div class="user-data-tagline-container">
                    <p>${bio}</p>
                </div>
                ${work ? `<div class="user-data-info"><b>WORK</b><p>${work}</p></div>` : ''}
                ${location ? `<div class="user-data-info"><b>LOCATION</b><p>${location}</p></div>` : ''}
                ${education ? `<div class="user-data-info"><b>EDUCATION</b><p>${education}</p></div>` : ''}
                <div class="user-data-info"><b>JOINED</b><p>${joinedDate}</p></div>
            </div>
        </div>`;
}
        ...
        <div class="post-card-body p-3">
            <div class="post-card-user-info d-flex gap-2 align-items-center">
                ...
                <div class="post-user-info">
                    ${createAuthorDropdownHTML(post.author)}
